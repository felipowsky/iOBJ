...
- (void)viewDidLoad
{
    ...
    camera.aspect = fabs(self.view.bounds.size.width / self.view.bounds.size.height);
...
{
    self.camera.aspect = fabs(self.view.bounds.size.width / self.view.bounds.size.height);
...
- (void)glkView:(GLKView *)view drawInRect:(CGRect)rect
{
    ...
    if (graphicObject && self.camera) {
        ...
        facesCount = (GLuint) graphicObject.mesh.faces.count;
    }
...
- (void)progressiveMeshSliderValue:(float)value
{
    ...
    self.percentageProgressiveLabel.text = [NSString stringWithFormat:@"%d%%", (int) value];
}
...
{
    FileListViewController *viewController = [UIViewController viewControllerInitialStoryboard:@"FileList" bundle:nil];
    
    viewController.selectedFile = self.loadedFile;
    //viewController.delegate = self;
    
    [self openViewController:viewController animation:CTHAnimationLeft modal:NO completion:^{
    }];
}
